...
void expression_parser::cleanup_expressions(bool final) {
  ...
  for (size_t i = virtual_start; i < expressions.size(); i++) {
    ...
    if (((expr = dynamic_cast<multi_filter_wrapper_expression*>(item)) != nullptr)) {
      if (virtual_start != i) {
        throw std::invalid_argument("virtual_start should always == i: " + std::to_string(virtual_start) + std::string(" != ") + std::to_string(i));
      }
      virtual_start++;
    }
  }
  ...
  for (size_t i = virtual_start; i < expressions.size(); i++) {
    ...
    if (((expr = dynamic_cast<not_expression*>(item)) != nullptr) && !(expr->expr)) {
  ...
  for (size_t i = virtual_start; i < expressions.size(); i++) {
    ...
    if (((e_expr = dynamic_cast<equivalent_expression*>(item)) != nullptr) && !(e_expr->left) && !(e_expr->right)) {
      e_expr->left = expressions[i - 1];
      e_expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
      i -= 2; // May need to goto restart here...
    } else if (((ne_expr = dynamic_cast<not_equivalent_expression*>(item)) != nullptr) && !(ne_expr->left) && !(ne_expr->right)) {
      ne_expr->left = expressions[i - 1];
      ne_expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
      i -= 2; // May need to goto restart here...
    } else if (((lt_expr = dynamic_cast<lt_expression*>(item)) != nullptr) && !(lt_expr->left) && !(lt_expr->right)) {
      lt_expr->left = expressions[i - 1];
      lt_expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
      i -= 2; // May need to goto restart here...
    } else if (((le_expr = dynamic_cast<le_expression*>(item)) != nullptr) && !(le_expr->left) && !(le_expr->right)) {
      le_expr->left = expressions[i - 1];
      le_expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
      i -= 2; // May need to goto restart here...
    } else if (((gt_expr = dynamic_cast<gt_expression*>(item)) != nullptr) && !(gt_expr->left) && !(gt_expr->right)) {
      gt_expr->left = expressions[i - 1];
      gt_expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
      i -= 2; // May need to goto restart here...
    } else if (((ge_expr = dynamic_cast<ge_expression*>(item)) != nullptr) && !(ge_expr->left) && !(ge_expr->right)) {
  ...
  for (size_t i = virtual_start; i < expressions.size(); i++) {
    ...
    if (((expr = dynamic_cast<and_expression*>(item)) != nullptr) && !(expr->left) && !(expr->right)) {
  ...
  for (size_t i = virtual_start; i < expressions.size(); i++) {
    ...
    if (((expr = dynamic_cast<or_expression*>(item)) != nullptr) && !(expr->left) && !(expr->right)) {
      expr->left = expressions[i - 1];
      expr->right = expressions[i + 1];
      expressions.erase(expressions.begin() + i + 1);
      expressions.erase(expressions.begin() + i - 1);
  ...
  if (final) {
    if (expressions.size() > (virtual_start + 1)) {
      throw std::invalid_argument("expressions is still too large! " + std::to_string(expressions.size()));
    }
    if (virtual_start) {
...
std::vector<expression_t> expression_parser::parse(std::string str) {
  ...
  for (i = 0; i < str.size(); i++) {
    ...
    switch (c) {
      case '(': {
        try {
          std::string substring = get_full_parentheses_statement(str.substr(i + 1));
          expression_parser child_parser(m_provider);
          std::vector<expression_t> expressions = child_parser.parse(substring); // shouldn't have multiple expressions in ()
          expression_t expr = expression_t(new grouped_expression(expressions[0]));
          add_expression(expr);
      ...
      case 'm': {
        ...
        expression_t expr = std::make_shared<match_function_expression>(expressions[0], std::make_shared<literal_value_expression>(std::make_shared<json_node>(trim(second_param))));
        add_expression(expr);
        break;
      }
      case 's': {
        ...
        expression_parser child_parser(m_provider);
        std::vector<expression_t> expressions = child_parser.parse(first_param);
        expression_t expr = std::make_shared<search_function_expression>(expressions[0], std::make_shared<literal_value_expression>(std::make_shared<json_node>(trim(second_param))));
        add_expression(expr);
        break;
      }
      case '[': {
      ...
      case ',': {
        // multiple filters
        // clean up current expressions and throw them into multi_filter_wrapper_expression.
      ...
      case '?': {
        if(dynamic_cast<multi_filter_wrapper_expression*>(expressions.back().get()) == nullptr) {
          throw std::invalid_argument("invalid '?' at index: " + std::to_string(i));
        }
        break;
      }
      default: {
